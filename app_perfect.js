...
if (remKm <= 1000) {
oilData.alert = `PrÃ³ximo cambio de aceite en ${remKm.toLocaleString()} km.`;
}
} else {
missingList.push('Ãšltimo cambio de aceite');
}
...
let tireData = { hasData: false, score: 100, remainingKm: cfg.tiresKm, detail: 'Sin historial de llantas', alert: null };
...
usedKm: kmUsed,
condition: condText,
detail: `${condText} â€¢ Restan ${remKm.toLocaleString()} km`
};
if (score < 25) {
tireData.alert = `La vida Ãºtil de llantas es inferior al 25% (restan ${remKm.toLocaleString()} km).`;
}
} else {
missingList.push('Cambio de llantas');
}
...
const docAvgScore = hasDocsData ? Math.round(docScores.reduce((a, b) => a + b, 0) / docScores.length) : 100;
if (!hasDocsData) missingList.push('DocumentaciÃ³n del vehÃ­culo');

const docData = {
hasData: hasDocsData,
score: docAvgScore,
details: docDetails.length > 0 ? docDetails.join(' â€¢ ') : 'Sin documentos registrados',
alerts: docAlerts
};
...
// CONFIDENCE INDEX (% Confiabilidad)
const componentsList = [
{ name: 'oil', data: oilData, weight: cfg.weights.oil },
{ name: 'tires', data: tireData, weight: cfg.weights.tires },
{ name: 'brakes', data: brakeData, weight: cfg.weights.brakes },
{ name: 'battery', data: batteryData, weight: cfg.weights.battery },
{ name: 'filters', data: filterData, weight: cfg.weights.filters },
{ name: 'belts', data: beltData, weight: cfg.weights.belts },
{ name: 'docs', data: docData, weight: cfg.weights.docs }
];
...
// GENERAL SCORE WEIGHTED CALCULATION
...
// Next Upcoming Service Highlight
...
<h2 style="font-size:1.2rem; color:var(--text-primary); margin-bottom:8px;">ðŸš— No hay vehÃ­culo activo</h2>
<p class="subtitle">Registra o selecciona un vehÃ­culo en la pestaÃ±a Garaje para visualizar el diagnÃ³stico de Salud del VehÃ­culo.</p>
...
const strokeOffset = circumference - (h.overallHealthPct / 100) * circumference;
...
<span><strong>Sin problemas crÃ­ticos.</strong> Todo funciona correctamente en tu automÃ³vil.</span>
</div>
</div>
`;
}

// Render Reliability Confidence Banner
let confidenceTipsHtml = '';
if (h.missingList.length > 0) {
confidenceTipsHtml = `
<div class="health-confidence-tips">
<strong>Para mejorar la precisiÃ³n del anÃ¡lisis registre:</strong>
<div style="margin-top:4px; display:flex; flex-wrap:wrap; gap:6px;">
${h.missingList.map(item => `<span class="health-chip">â€¢ ${escapeHtml(item)}</span>`).join('')}
</div>
</div>
`;
}
...
<span>ðŸš—</span> ${escapeHtml(veh.name || 'VehÃ­culo')} (${veh.year || ''})
</div>
...
<div class="health-gauge-val">${h.overallHealthPct}%</div>
</div>
<span style="font-size:0.75rem; color:#94a3b8; margin-top:6px; font-weight:700; text-transform:uppercase;">Estado</span>
</div>
...
${h.allSmartAlerts.length === 0 ? 'Sin problemas crÃ­ticos detectados.' : `Se han detectado ${h.allSmartAlerts.length} observaciÃ³n(es) de mantenimiento.`}
</div>

<div class="health-next-service">
<strong>PrÃ³ximo servicio:</strong> ${escapeHtml(h.nextServiceText)}
</div>
</div>
</div>
...
<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#38bdf8" stroke-width="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
Confiabilidad del AnÃ¡lisis
</div>
<strong style="color:#38bdf8; font-size:0.9rem;">${h.confidencePct}%</strong>
</div>
...
const diagnosticPrompt = `Genera un Diagnóstico Integral de Salud Vehicular, Análisis de Desgaste, Recomendaciones de Mantenimiento Preventivo y Predicciones Técnicas para el siguiente vehículo:
- Vehículo: ${veh.name} (${veh.year}, ${veh.type}, Odómetro: ${(Number(veh.km)||0).toLocaleString()} KM, Placa: ${veh.plate || 'N/A'})
- Historial Reciente de Servicios:
${servicesSummary}
- Recordatorios y Alertas Pendientes:
${remindersSummary}

Entrega una revisión clara estructurada en:
1. Estado General de Salud y Desgaste Estimado
2. Recomendaciones Mecánicas Inmediatas
3. Predicciones Preventivas para el Próximo Cambio de Aceite, Frenos y Distribución.`;

const input = document.getElementById('aiUserQuestion');
if (input) input.value = 'Generar Diagnóstico de Salud Vehicular Completo';

await askAIAssistantDirect(diagnosticPrompt);

if (logBox) {
logBox.style.display = 'block';
logBox.innerHTML = '<span style="color:#30d158; font-weight:700;">✓ Diagnóstico generado exitosamente con los datos actuales del vehículo. Revisa el resultado abajo.</span>';
}
...
Invertido este aÃ±o: ${formatCurrency(h.expensesSummary.yearTotal)}<br>
Promedio mensual: ${formatCurrency(h.expensesSummary.monthlyAvg)}<br>
Total histÃ³rico: ${formatCurrency(h.expensesSummary.totalHistoric)}
</div>
</div>
<div class="health-card-footer">
<span style="font-size:0.75rem; color:var(--text-secondary);">Reportes</span>
